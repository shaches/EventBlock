...
/**
 * Reflection-based coverage of the private helpers that drive {@link
 * LegacyBlocksMigrator#migrateBlocks}:
 *
 * <ul>
 *   <li>{@code classify(text, chestMap)} &mdash; turns a raw legacy pool string into a {@code
 *       "kind|value"} accumulator key.
 *   <li>{@code detectHeaderEnd(list)} &mdash; finds where the positional header (name / color /
 *       style / length) ends and pool entries begin.
 *   <li>{@code buildEntryMap(accKey, weight)} &mdash; materializes a weighted-schema map from the
 *       accumulator key and count.
 * </ul>
 *
 * These are pure logic and deliberately avoid {@code Bukkit.getLogger()}, so they run without a
 * Bukkit server context.
 */
class LegacyBlocksMigratorFlattenTest {
  ...
  static void reflectPrivates() throws Exception {
    CLASSIFY = LegacyBlocksMigrator.class.getDeclaredMethod("classify", String.class, Map.class);
    CLASSIFY.setAccessible(true);

    DETECT_HEADER_END = LegacyBlocksMigrator.class.getDeclaredMethod("detectHeaderEnd", List.class);
    DETECT_HEADER_END.setAccessible(true);

    BUILD_ENTRY_MAP =
        LegacyBlocksMigrator.class.getDeclaredMethod("buildEntryMap", String.class, int.class);
  ...
  void classifyBlockFallback() throws Exception {
    ...
    assertThat(classify("some_weird_id", Collections.emptyMap())).isEqualTo("block|some_weird_id");
  }
  ...
  @Test
  @DisplayName(
      "buildEntryMap: loot_table kind preserves the full loot-table string (colon-in-value)")
  ...
  @Test
  @DisplayName(
      "buildEntryMap: command kind preserves the full command (even with '|' in the template)")
