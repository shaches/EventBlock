...
/**
 * Regression net for the Phase 4.1 publication-safety contract on {@link Level#levels}: writers
 * stage a fresh list and call {@link Level#replaceAll(List)} once, so concurrent readers either see
 * the entire pre-call snapshot or the entire post-call snapshot - never a half-populated mid-state.
 * Pre-Phase-4.1 the field was a public {@link ArrayList} that {@code ConfigManager.loadBlocks}
 * cleared and refilled in place, exposing a {@link java.util.ConcurrentModificationException}
 * window for the async {@code IslandParticleTask} / {@code OBP} placeholder consumers that
 * indirectly read {@link Level#get(int)}.
 *
 * <p>If a future maintainer reverts to in-place mutation ({@code clear() + addAll()}) instead of an
 * atomic {@link Level#replaceAll(List)}, the {@link #replaceAllAtomicConcurrent} test will see a
 * partial size or a partially-iterated snapshot and fail.
 */
class LevelPublicationTest {
  ...
  @Test
  @DisplayName(
      "concurrent readers never observe a partial size during alternating replaceAll calls")
                ...
                for (Level lvl : snap) {
                  if (lvl == null) {
                    failure.compareAndSet(
                        null, "saw null entry in snapshot of size " + snap.size());
                    return;
                  }
                }
              }
