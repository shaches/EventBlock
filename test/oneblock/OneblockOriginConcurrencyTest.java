...
/**
 * JMM regression tests for the Phase 2 thread-safety refactor: the four former {@code
 * Oneblock.x/y/z/offset} static fields were folded into a single {@link IslandOrigin} reference
 * swapped via {@link java.util.concurrent.atomic.AtomicReference#updateAndGet}. These tests hammer
 * {@code Oneblock.ORIGIN} from competing threads and assert that readers only ever observe
 * self-consistent snapshots.
 *
 * <p>The concurrency test is timing-sensitive but avoids flakiness by hammering for a fixed
 * iteration budget rather than a wall-clock deadline, and by using a latch so the reader cannot
 * race ahead of the writer's first swap. No Bukkit / Plugin instantiation is required because only
 * the static {@code ORIGIN} AtomicReference is touched.
 */
class OneblockOriginConcurrencyTest {
  ...
  void readerSeesOnlyConsistentSnapshots() throws InterruptedException {
    ...
    assertThat(tornReads.get()).as("Reader should never see a torn ORIGIN snapshot").isZero();
  }
