 ...
 *
 * <ul>
 *   <li>{@link Oneblock#plugin} is a Mockito mock with the {@code cache} field assigned to a mock
 *       {@link PlayerCache} and {@code findNearestRegionId(Location)} stubbed when the {@code
 *       _by_position} dispatch is exercised. Restored in {@link #restorePlugin}.
...
class OBPDispatchTest {
  ...
  @Test
  @DisplayName(
      "%OB_lvl_lenght% (legacy typo preserved as alias) returns the current level's length")
  ...
  @Test
  @DisplayName(
      "%OB_lvl_lenght% legacy spelling logs a deprecation warning exactly once per session")
  void lvlLenghtLegacyAliasLogsDeprecationOnce() {
    ...
    org.mockito.Mockito.verify(Oneblock.plugin.getLogger()).warning(msg.capture());
