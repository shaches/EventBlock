...
/**
 * Security-critical coverage: {@link RewardManager#executeRewards} refuses to dispatch any console
 * command unless the player's in-game name matches the private {@code SAFE_PLAYER_NAME} pattern. A
 * regression that loosens this regex would re-open the command-injection vector that this gate
 * closes.
 *
 * <p>The rest of {@code executeRewards} depends on {@code Oneblock.plugin}, {@code
 * Bukkit.getConsoleSender()}, and {@code Bukkit.dispatchCommand} which require a live Bukkit server
 * to exercise meaningfully. Those paths are left for an integration test (outside the scope of the
 * Phase 5 unit suite).
 */
