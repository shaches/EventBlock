...
/**
 * Coverage for {@link LegacyYamlPlayerDataStore}, the one-shot pre-1.x YAML player-data migrator.
 * Phase 5.3 lifts this class from {@code 0%} line coverage. The migrator runs only on first start
 * when no {@code PlData.json} exists yet, but a regression here silently drops every legacy
 * server's player roster on its post-upgrade reboot - the precise data-loss class Phase 5 was
 * sequenced to fence.
 *
 * <p>The class-load-time evaluation of the {@code static File f} field is handled by the same trick
 * used in {@link JsonPlayerDataStoreTest}: install a Mockito-mocked {@link Oneblock#plugin} whose
 * {@code getDataFolder()} returns a class-scoped {@link TempDir}, then force the class to
 * initialise via {@link Class#forName} before any test method runs.
 *
 * <p>Nick-to-UUID resolution funnels through {@link Utils#getOfflinePlayerByName} which
 * dereferences {@link org.bukkit.Bukkit#getOfflinePlayers()} and only works on a real server. Each
 * test that exercises a populated legacy file therefore wraps the {@code read()} call in {@link
 * MockedStatic Mockito.mockStatic(Utils.class)} and stubs the resolver to return either a fake
 * {@link OfflinePlayer} (for resolved nicks) or {@code null} (for the unresolved-nick warning
 * branch).
 */
class LegacyYamlPlayerDataStoreTest {
  ...
  @Test
  @DisplayName(
      "multi-island legacy file: each island gets its own PlayerInfo with the right lvl/breaks")
  void multipleIslandsResolveIndependently() throws Exception {
    ...
    assertThat(read).extracting(p -> p.uuid).containsExactly(aliceUuid, bobUuid);
  ...
  void unresolvedNickIsLoggedAndSkipped() throws Exception {
    ...
    ArrayList<PlayerInfo> read;
    try (MockedStatic<Utils> ms = Mockito.mockStatic(Utils.class)) {
      ms.when(() -> Utils.getOfflinePlayerByName("ghost")).thenReturn(null);
      read = LegacyYamlPlayerDataStore.read();
    }
