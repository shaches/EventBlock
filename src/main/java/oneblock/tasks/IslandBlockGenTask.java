...
/**
 * Main-thread block-generation pulse: every four seconds (80 ticks) it walks every online island
 * player and, if the generation block is air, delegates to {@link Oneblock#generateBlock} to
 * (re)materialise it. Also enforces the {@code protection} flag - players outside their cell are
 * teleported back via {@code /ob j} after a {@link Messages#protection} message.
 *
 * <p>Renamed from the inner class {@code Oneblock.Task} (whose body carried a single inline
 * comment, "// SubBlockGen", indicating the intent) in Phase 3.4. Runs on the main thread because
 * {@link Block#getType} and {@code generateBlock} both need synchronous world access.
 */
public final class IslandBlockGenTask implements Runnable {
  ...
  @Override
  public void run() {
    for (Map.Entry<Player, int[]> entry : plugin.cache.getEntries()) {
      Player player = entry.getKey();
      if (player.getWorld() != Oneblock.getWorld()) continue;
      final UUID uuid = player.getUniqueId();
      final int[] result = entry.getValue();
