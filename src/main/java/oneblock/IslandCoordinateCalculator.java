...
public final class IslandCoordinateCalculator {
  ...
  /**
   * Spatial index: packed (cellX, cellZ) &rarr; island id. Each island occupies exactly one cell of
   * size {@code Oneblock.offset}, so exact-hit lookups are O(1). Populated lazily on first use
   * after {@link #invalidateCellIndex()} is called by a {@link PlayerInfo} mutation.
   */
  ...
  public static void invalidateCellIndex() {
    cellIndex = null;
  }
  ...
  private static ConcurrentMap<Long, Integer> ensureIndex() {
    ...
    synchronized (IslandCoordinateCalculator.class) {
      ...
      int offset = o.offset();
      if (offset == 0) {
        cellIndex = new ConcurrentHashMap<>();
        return cellIndex;
      }
      int half = offset >> 1;
  ...
  /**
   * @return the id of the island whose cell contains {@code loc}, or the spatially-nearest id if
   *     the location is outside every occupied cell.
   *     <p><b>Caveat:</b> returns {@code 0} also for the degenerate cases {@code loc == null},
   *     {@code PlayerInfo.size() == 0}, and {@code offset == 0}, which collides with the id of the
   *     first island. Callers that need to distinguish "no islands" from "nearest is id 0" must
   *     guard on {@code PlayerInfo.size() > 0} before interpreting the return value.
   */
  ...
  /**
   * Spiral-ring island coordinate resolver. Uses iterative walk for ids &le; 30 and closed-form
   * ring math for larger ids.
   */
