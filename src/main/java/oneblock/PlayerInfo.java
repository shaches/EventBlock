...
public final class PlayerInfo {
   ...
   *
   * <p>Phase 3.5: Encapsulated behind getter/setter methods to enable testability and DI. Direct
   * field access is deprecated; use {@link #getList()} and {@link #setList(List)} instead.
   */
  private static final List<PlayerInfo> list = new CopyOnWriteArrayList<>();

  /**
   * Get the player info list. Returns a snapshot copy for thread-safe iteration. Phase 3.5:
   * Encapsulates direct list access.
   */
  public static List<PlayerInfo> getList() {
    return new CopyOnWriteArrayList<>(list);
  }

  /**
   * Set the player info list. Used during data loading and migration. Phase 3.5: Encapsulates
   * direct list access.
   */
  public static void setList(List<PlayerInfo> newList) {
    list.clear();
    list.addAll(newList);
    UUID_INDEX.clear();
    for (int i = 0; i < list.size(); i++) {
      registerInIndex(list.get(i), i);
    }
    IslandCoordinateCalculator.invalidateCellIndex();
    TOP_VERSION.incrementAndGet();
  }

  /** Check if the player info list is empty. Phase 3.5: Encapsulates direct list access. */
  public static boolean isListEmpty() {
    return list.isEmpty();
  }

  /** Get the size of the player info list. Phase 3.5: Encapsulates direct list access. */
  public static int getListSize() {
    return list.size();
  }

  /**
   * Get the internal list reference for direct mutation. Only for use by PlayerInfo's own methods
   * that need to modify the list in place. External callers should use getList() for read-only
   * access.
   */
  private static List<PlayerInfo> getInternalList() {
    return list;
  }
  ...
  @SuppressFBWarnings(
      ...
      justification =
          "Public field as part of the persistence schema (JSON/YAML serialization). "
              + "This UUID is the primary key for player data and needs to be accessible for "
  ...
  @SuppressFBWarnings(
      value = "PA_PUBLIC_PRIMITIVE_ATTRIBUTE",
      justification =
          "Public field as part of the persistence schema (JSON/YAML serialization). This level ID"
              + " is read and written by the storage layer and needs to be accessible for"
              + " reflection-based serialization. Access is controlled by the class design.")
  ...
  @SuppressFBWarnings(
      value = "PA_PUBLIC_PRIMITIVE_ATTRIBUTE",
      justification =
          "Public field as part of the persistence schema (JSON/YAML serialization). This field is"
              + " read and written by the storage layer and needs to be accessible for"
              + " reflection-based serialization. Access is controlled by the class design.")
   ...
   */
  @SuppressFBWarnings(
      value = {"PA_PUBLIC_PRIMITIVE_ATTRIBUTE", "AT_STALE_THREAD_WRITE_OF_PRIMITIVE"},
      justification =
          "Legacy field kept for storage migration (Phase 1). Public for serialization"
              + " compatibility. Do not use in new code; prefer currentLevelId. Will be removed"
              + " once migration is complete. Thread-safety warnings are acceptable because this is"
              + " a legacy field only used by main-thread code paths during migration.")
  public int lvl = 0;
  ...
  @SuppressFBWarnings(
      ...
      justification =
          "Legacy field kept for storage migration (Phase 1). Public for serialization"
              + " compatibility. Do not use in new code; prefer taskProgress. Will be removed once"
              + " migration is complete. Thread-safety warnings are acceptable because this is a"
              + " legacy field only used by main-thread code paths during migration.")
  public int breaks = 0;
  ...
  @SuppressFBWarnings(
      ...
      justification =
          "Public field as part of the persistence schema (JSON/YAML serialization). "
              + "This BossBar reference is managed by the plugin and needs to be accessible for "
              + "reflection-based serialization. Access is controlled by the class design.")
  public transient BossBar bar = null;
  ...
  @SuppressFBWarnings(
      ...
      justification =
          "Legacy field synchronization for Phase 1 migration. The lvl field is a legacy integer"
              + " kept for storage migration compatibility. Non-atomic operations are acceptable"
              + " because this method is called from the main thread after all mutations, and the"
  ...
  @SuppressFBWarnings(
      value = "AT_NONATOMIC_OPERATIONS_ON_SHARED_VARIABLE",
      justification =
          "Legacy field increment for Phase 1 migration. The lvl field is a legacy integer kept for"
              + " storage migration compatibility. Non-atomic operations are acceptable because"
  ...
  public Level advanceToLevel(String nextLevelId) {
    // Phase 6: Validate level ID to prevent injection attacks
  ...
  public static void removeBarFor(Player p) {
    if (getInternalList().isEmpty()) return;
  ...
  public static void set(int id, PlayerInfo pInf) {
    if (id < getInternalList().size()) getInternalList().set(id, pInf);
    else getInternalList().add(pInf);
  ...
  public static void replaceAll(List<PlayerInfo> newList) {
    ...
    UUID_INDEX.clear();
    if (newList != null) {
      getInternalList().addAll(newList);
  ...
  private static int getNull() {
    for (int i = 0; getInternalList().size() > i; i++)
      if (getInternalList().get(i).uuid == null) return i;
    return getInternalList().size();
  }
  ...
  public static final Comparator<PlayerInfo> COMPARE_BY_LVL =
      new Comparator<PlayerInfo>() {
        @SuppressFBWarnings(
            value = "PA_PUBLIC_PRIMITIVE_ATTRIBUTE",
            justification =
                "Anonymous comparator class accesses public fields of PlayerInfo for sorting. These"
                    + " fields are part of the persistence schema and are intentionally public for"
                    + " serialization. The comparator is a private implementation detail of"
                    + " PlayerInfo.")
        @Override
        ...
        private int levelIndex(PlayerInfo inf) {
          ...
          for (int i = 0; i < ordered.size(); i++) {
            if (ordered.get(i).id.equals(inf.currentLevelId)) return i;
          }
          return 0;
        }
      };
