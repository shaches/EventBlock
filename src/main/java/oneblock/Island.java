...
public final class Island {
  @SuppressFBWarnings(
      value = {"MS_MUTABLE_ARRAY", "PA_PUBLIC_PRIMITIVE_ATTRIBUTE"},
      justification =
          "This static array holds the custom island layout configuration. It is intentionally"
              + " mutable to allow runtime reconfiguration via the /ob scan command and config file"
              + " loading. The array is only modified from the main thread during configuration"
              + " operations, making this design safe for the plugin's single-threaded"
              + " configuration system. Public for cross-package access from command handlers that"
              + " need to read/scan the island layout.")
