...
/**
 * {@code /ob islands [true|false | set_my_by_def | default]} - admin. Multi-mode toggle for the
 * welcome-island system:
 *
 * <ul>
 *   <li>{@code true|false} - bool toggle for the {@code island_for_new_players} flag.
 *   <li>{@code set_my_by_def} - snapshot the sender's current island layout into {@code
 *       config.yml#custom_island} so future spawn islands use it as the template (player-only,
 *       1.13+ only).
 *   <li>{@code default} - clear any custom layout and revert to the built-in welcome island (1.13+
 *       only).
 * </ul>
 *
 * <p>Behaviour-equivalent to the legacy {@code "islands"} admin case extracted in Phase 3.5b.4.
 */
  ...
  @SuppressFBWarnings(
      ...
      justification =
          "Admin-only command that intentionally modifies global plugin state. The static"
              + " Island.custom field is part of the plugin's global configuration that admin"
  ...
  public boolean execute(CommandContext ctx) {
    ...
    if (args[1].equals("true") || args[1].equals("false")) {
      ...
      ctx.sender()
          .sendMessage(
              ChatColor.GREEN
                  + "Island_for_new_players = "
                  + Oneblock.settings().islandForNewPlayers);
      return true;
    }
    if (args[1].equals("set_my_by_def")) {
      ...
      if (PlayerInfo.getId(uuid) != -1) {
        ...
        ctx.sender()
            .sendMessage(ChatColor.GREEN + "A copy of your island has been successfully saved!");
