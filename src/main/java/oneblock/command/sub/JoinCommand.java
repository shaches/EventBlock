...
/**
 * {@code /ob j} (alias) and {@code /ob join} - user-facing. Either teleports the sender to their
 * existing island, or - on first call - creates a fresh island, registers it with WorldGuard if
 * enabled, places the welcome layout, kicks off the BossBar, and only then teleports.
 *
 * <p>Behaviour-equivalent to the legacy {@code "j"}/{@code "join"} switch case extracted in Phase
 * 3.5b. Refuses politely (and prompts the admin to run {@code /ob set} first) when {@code offset}
 * is zero or the configured world has not yet loaded. Requires a {@link org.bukkit.entity.Player}
 * sender; console has no location to teleport.
 */
public final class JoinCommand implements Subcommand {
  ...
  public boolean execute(CommandContext ctx) {
    if (Oneblock.getOffset() == 0 || Oneblock.getWorld() == null) {
      ctx.sender()
          .sendMessage(
              ChatColor.YELLOW + "First you need to set the reference coordinates '/ob set'.");
      return true;
    }
    ...
    if (plID == -1) {
      ...
      inf.createBar(Oneblock.getBarTitle(ctx.player(), inf));
    } else {
      int[] result = plugin.getIslandCoordinates(plID);
      X_pl = result[0];
      Z_pl = result[1];
