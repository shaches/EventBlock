...
import oneblock.security.RateLimiter;
...
public final class CommandHandler implements CommandExecutor {
  ...
  static java.util.Set<String> getRegisteredSubcommands() {
  ...
  public static boolean idresetCommand(OfflinePlayer pl) {
    ...
    if (OBWorldGuard.isEnabled()) {
      ...
      if (ctx != null) {
        ctx.plugin().worldGuard.removeMember(uuid, PlId);
      }
    }
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    // Phase 6: Rate limit player commands to prevent spam/DoS
    if (player != null && !RateLimiter.tryConsume(player.getUniqueId())) {
      sender.sendMessage(ChatColor.RED + "You are sending commands too quickly. Please wait.");
      return true;
    }

    String parametr = args[0].toLowerCase(Locale.ROOT);
    ...
    if (routed != null) {
      ...
      return routed.execute(new CommandContext(sender, player, args, PluginContext.get().plugin()));
    }
    ...
    PluginContext ctx = PluginContext.get();
    String version = ctx != null ? ctx.plugin().version : "unknown";
    sender.sendMessage(
