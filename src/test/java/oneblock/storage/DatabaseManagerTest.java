...
/**
 * Round-trip coverage for {@link DatabaseManager}, the SQL-backed storage path used when {@code
 * database.type} is set to {@code h2} or {@code mysql} in {@code config.yml}. Phase 5.6 lifts the
 * class from {@code 0%} line coverage; a regression here drops every SQL-backed server's roster on
 * the post-upgrade reboot - same data-loss class as Phase 5.3.
 *
 * <p>The tests use H2 in <b>file mode</b> via the production {@link DatabaseManager#initialize()}
 * path, not an in-memory shim. H2 is on the runtime classpath of the project (pom.xml: {@code
 * com.h2database:h2:2.4.240} at runtime scope), and the production code already builds the H2 JDBC
 * URL from {@code Oneblock.plugin.getDataFolder()}. We make {@code getDataFolder()} return a
 * per-test {@link TempDir} so each test starts from a clean H2 database file.
 *
 * <p>Per-test isolation:
 *
 * <ul>
 *   <li>{@link #installPerTestPlugin} installs a fresh Mockito mock of {@link Oneblock} with {@code
 *       getDataFolder()} -> {@code @TempDir} and {@code getLogger()} -> mock {@link Logger}, then
 *       resets every {@code DatabaseManager} static config field to its documented default and
 *       calls {@link DatabaseManager#initialize()}.
 *   <li>{@link #closePool} releases the Hikari pool (and therefore H2's file lock) so the next test
 *       can use a fresh tempdir.
 *   <li>Class-level {@link #snapshotStatics} / {@link #restoreStatics} captures and restores the
 *       original static values so this test class doesn't leak DB config into other test classes
 *       that run later in the same JVM.
 * </ul>
 *
 * <p>Validation-error paths ({@code dbType=mysql} with malformed host / port / database name)
 * exercise the production allow-list patterns without needing a real MySQL: the {@code
 * IllegalArgumentException} is thrown inside the {@code initialize()} try/catch, which logs at
 * {@code SEVERE} and sets {@code dataSource = null}. Tests assert post-conditions ({@link
 * DatabaseManager#isConnected()} is false) and verify the warning text via the mock logger.
 */
class DatabaseManagerTest {
  ...
  static void snapshotStatics() {
    ...
    DatabaseManager.Config savedConfig = DatabaseManager.getConfig();
    savedDbType = savedConfig.dbType;
    savedHost = savedConfig.host;
    savedPort = savedConfig.port;
    savedDatabase = savedConfig.database;
    savedUsername = savedConfig.username;
    savedPassword = savedConfig.password;
    savedUseSSL = savedConfig.useSSL;
    savedAutoReconnect = savedConfig.autoReconnect;
  ...
  static void restoreStatics() {
    ...
    DatabaseManager.Config restoreConfig = new DatabaseManager.Config();
    restoreConfig.dbType = savedDbType;
    restoreConfig.host = savedHost;
    restoreConfig.port = savedPort;
    restoreConfig.database = savedDatabase;
    restoreConfig.username = savedUsername;
    restoreConfig.password = savedPassword;
    restoreConfig.useSSL = savedUseSSL;
    restoreConfig.autoReconnect = savedAutoReconnect;
    DatabaseManager.setConfig(restoreConfig);
  }
  ...
  void installPerTestPlugin() {
    ...
    DatabaseManager.Config defaultConfig = new DatabaseManager.Config();
    defaultConfig.dbType = "h2";
    defaultConfig.host = "localhost";
    defaultConfig.port = 3306;
    defaultConfig.database = "oneblock";
    defaultConfig.username = "root";
    defaultConfig.password = "";
    defaultConfig.useSSL = false;
    defaultConfig.autoReconnect = true;
    DatabaseManager.setConfig(defaultConfig);
  }
  ...
  void initializeJsonIsNoOp() {
    DatabaseManager.getConfig().dbType = "json";
    DatabaseManager.initialize();
    assertThat(DatabaseManager.isConnected()).isFalse();
    verify(pluginLogger, atLeastOnce())
        .info(org.mockito.ArgumentMatchers.contains("Using JSON storage"));
  }
  ...
  void initializeH2OpensPool() {
    ...
    verify(pluginLogger, atLeastOnce())
        .info(org.mockito.ArgumentMatchers.contains("Database initialized successfully"));
  }

  @Test
  @DisplayName(
      "initialize with dbType=mysql + invalid host fails the allow-list and falls back (isConnected"
          + " false)")
  void initializeMysqlInvalidHost() {
    DatabaseManager.getConfig().dbType = "mysql";
    DatabaseManager.getConfig().host =
        "evil host;DROP TABLE foo;--"; // contains spaces and semicolons
    DatabaseManager.initialize();
    assertThat(DatabaseManager.isConnected()).isFalse();
  ...
  void initializeMysqlInvalidPort() {
    DatabaseManager.getConfig().dbType = "mysql";
    DatabaseManager.getConfig().port = 70000;
    DatabaseManager.initialize();
    assertThat(DatabaseManager.isConnected()).isFalse();
    verify(pluginLogger, atLeastOnce())
        .info(org.mockito.ArgumentMatchers.contains("Using JSON storage"));
  ...
  void initializeMysqlInvalidDatabase() {
    DatabaseManager.getConfig().dbType = "mysql";
    DatabaseManager.getConfig().database = "schema; DROP TABLE foo";
    DatabaseManager.initialize();
    assertThat(DatabaseManager.isConnected()).isFalse();
  }
  ...
  void saveUpsertsExistingRow() {
    ...
    UUID owner = UUID.randomUUID();
    PlayerInfo p1 = new PlayerInfo(owner);
    p1.lvl = 1;
    p1.currentLevelId = "level_1";
    ...
    PlayerInfo p2 = new PlayerInfo(owner);
    p2.lvl = 9;
    p2.currentLevelId = "level_9";
  ...
  void saveSkipsNullEntries() {
    ...
    assertThat(loaded).extracting(p -> p.uuid).containsExactlyInAnyOrder(a, c);
  }
  ...
  void mysqlValidationErrorLogged() {
    DatabaseManager.getConfig().dbType = "mysql";
    DatabaseManager.getConfig().host = "evil host"; // contains a space
    DatabaseManager.initialize();
