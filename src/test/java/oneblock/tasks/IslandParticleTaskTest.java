...
/**
 * Unit coverage for {@link IslandParticleTask#collectSpawns} - the async, pure-function half of the
 * Phase 4.3 split. Exercises the cache-iteration semantics (skip null coordinates, emit four
 * corners per registered player, base-Y offset of +0.5) without spinning up a Bukkit server. The
 * main-thread dispatch via {@code Bukkit.getScheduler().runTask} is gated by Phase 5.6 once the
 * Mockito-Bukkit harness is available.
 *
 * <p>The {@link PlayerCache} is mocked so we control exactly which coordinates each player resolves
 * to. {@link World} is also mocked because {@link Location} only stores a {@link World} reference
 * and never dereferences it during construction.
 */
class IslandParticleTaskTest {
  ...
  void emptyCacheReturnsEmptyList() {
    ...
    when(cache.getEntries()).thenReturn(Collections.emptyList());

    List<Location> spawns =
        IslandParticleTask.collectSpawns(cache, Mockito.mock(World.class), BASE_Y);
  ...
  void singlePlayerProducesFourCorners() {
    ...
    when(cache.getEntries()).thenReturn(List.of(Map.entry(player, new int[] {100, 200, 0})));
    ...
    assertThat(spawns).extracting(Location::getY).containsOnly(EXPECTED_BASE_Y);
    // Every produced location references the same world the caller passed.
    assertThat(spawns).extracting(Location::getWorld).containsOnly(world);
  }
  ...
  void skipsPlayersWithNullCoordinates() {
    ...
    when(cache.getEntries())
        .thenReturn(
            Arrays.asList(
                Map.entry(resolved, new int[] {0, 0, 0}),
                new java.util.AbstractMap.SimpleEntry<>(unresolved, null)));
  ...
  void twoPlayersProduceEightLocations() {
    ...
    when(cache.getEntries())
        .thenReturn(
            Arrays.asList(
                Map.entry(a, new int[] {0, 0, 0}), Map.entry(b, new int[] {500, -300, 1})));
