...
/**
 * Coverage for {@link IslandCoordinateCalculator#findNearestRegionId(Location)} and the lazy {@code
 * cellIndex} cache that backs it. The existing {@link IslandCoordinateCalculatorTest} already pins
 * the iter/hybrid spiral correctness to a 500-id brute-force reference; this slice fills the {@code
 * findNearestRegionId} gap (~0% line coverage on the pre-Phase-5.2 tree).
 *
 * <p>The function is the bridge between a player {@link Location} and the per-island {@link
 * PlayerInfo} slot - every island-protection lookup, the placeholder {@code %OB_lvl_by_position%}
 * chain, and the WorldGuard region resolver all funnel through it. A regression here would either
 * silently route every protection check to island 0 or crash with an NPE in the lazy-init path;
 * both classes are covered below.
 *
 * <p>Tests build a deterministic 5-island layout in linear mode (offset=100, base=(0,0), so islands
 * sit at X=0,100,200,300,400 with Z=0) plus one circle-mode scenario for the spiral-fallback
 * branch. {@link PlayerInfo#replaceAll(List)} populates the slot list and automatically calls
 * {@link IslandCoordinateCalculator#invalidateCellIndex()} to keep the cache honest.
 */
class IslandCoordinateNearestRegionTest {
  ...
  @Test
  @DisplayName(
      "cell-index miss with halfDiameterSquared early-break: a Location very close to id 0"
  ...
  void cellMissEarlyBreak() {
    ...
    assertThat(IslandCoordinateCalculator.findNearestRegionId(locAt(50_000, 50_000)))
        .isBetween(0, 4);
  }
