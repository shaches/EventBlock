...
/**
 * Coverage for {@link OBP#onRequest(OfflinePlayer, String)}, the PlaceholderAPI hook used by every
 * server's sidebar / scoreboard / chat formatter to render {@code %OB_*%} tokens. Phase 5.4 lifts
 * the class from {@code 0%} line coverage; a regression here is a silent empty-string class - PAPI
 * swallows {@code null} returns, so a misrouted dispatch wouldn't even log a warning.
 *
 * <p>Three mock layers cooperate per test:
 *
 * <ul>
 *   <li>{@link PluginContext} is initialized with a Mockito mock {@link Oneblock} with the {@code
 *       cache} field assigned to a mock {@link PlayerCache} and {@code
 *       findNearestRegionId(Location)} stubbed when the {@code _by_position} dispatch is exercised.
 *       Restored in {@link #restorePlugin}.
 *   <li>{@link PlayerInfo#list} is populated per test via {@link PlayerInfo#replaceAll(List)} and
 *       restored in {@link #restorePlayerInfo}. {@link Level#max} and {@link Level#snapshot()} are
 *       likewise snapshotted so per-test level setups don't leak.
 *   <li>Tests that exercise {@code owner_name}, {@code owner_online}, {@code top_<n>_name} or any
 *       {@code _by_position} branch open a {@link MockedStatic Mockito.mockStatic(Bukkit.class)}
 *       and stub {@link Bukkit#getOfflinePlayer(UUID)} / {@link Bukkit#getPlayer(UUID)} to return a
 *       fake {@link OfflinePlayer} or {@link Player}. The stubbed lookup never reaches the real
 *       Bukkit server which would NPE on {@code Bukkit.server == null}.
 * </ul>
 */
class OBPDispatchTest {
  ...
  void prepareFixture() throws Exception {
    ...
    mockPlugin = mock(Oneblock.class);
    ...
    savedPlayerInfoList = new ArrayList<>(PlayerInfo.getList());
  ...
  @Test
  @DisplayName("%OB_lvl_length% returns the current level's length")
  void lvlLengthReturnsLevelLength() {
    UUID owner = UUID.randomUUID();
    putPlayer(owner, 0, 0);
  ...
  void playerCountDelegatesToCache() {
    UUID owner = UUID.randomUUID();
    putPlayer(owner, 0, 0);
    when(mockCache.getPlayers())
        .thenReturn(Arrays.asList(mock(Player.class), mock(Player.class), mock(Player.class)));
  ...
  @Test
  @DisplayName(
      "%OB_owner_online% returns 'online' when Bukkit.getPlayer resolves a non-null Player")
  ...
  @Test
  @DisplayName(
      "%OB_top_X_name% with a non-numeric position returns null (NumberFormatException caught)")
  ...
  @Test
  @DisplayName(
      "%OB_lvl_by_position% routes to the nearest island's owner's lvl via Bukkit.getOfflinePlayer"
          + " recursion")
  void byPositionRoutesToOwnerLvl() {
    ...
    when(mockPlugin.findNearestRegionId(loc)).thenReturn(0);
