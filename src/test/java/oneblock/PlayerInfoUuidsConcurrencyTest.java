...
/**
 * Regression net for the Phase 4.4 swap of {@link PlayerInfo#uuids} from a plain {@code ArrayList}
 * to a {@link java.util.concurrent.CopyOnWriteArrayList}: the async {@code PlayerDataSaveTask} -
 * which calls {@link oneblock.storage.JsonPlayerDataStore#write} and {@link
 * oneblock.storage.DatabaseManager#save} - iterates every island's {@code uuids} list while the
 * main thread can concurrently mutate it via {@link PlayerInfo#addInvite(UUID)}, {@link
 * PlayerInfo#removeInvite(UUID)} or the invitee-promotion branch of {@link
 * PlayerInfo#removeUUID(UUID)}.
 *
 * <p>Pre-Phase-4.4 this was a documented latent race that the codebase happened to dodge because
 * the save runs every 6000 ticks (5 minutes) and the kick/accept/idreset commands are rare; an
 * unlucky overlap would surface as a {@link java.util.ConcurrentModificationException} thrown out
 * of an enhanced {@code for} on the async save thread, losing the entire save tick's worth of data.
 *
 * <p>The test exercises the same access pattern - a reader iterating the list while a writer
 * alternates {@code add} + {@code remove} - and asserts no exception was captured.
 */
        ...
        new Thread(
            () -> {
              try {
                while (!stopReader.get()) {
                  String joined =
                      inf.uuids.stream().map(UUID::toString).reduce("", (a, b) -> a + "," + b);
