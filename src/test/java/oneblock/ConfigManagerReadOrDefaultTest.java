...
/**
 * Covers the five {@link ConfigManager#readOrDefault} overloads (String / int / double / boolean /
 * List&lt;String&gt;). The contract:
 *
 * <ol>
 *   <li>If the key is absent or is the wrong YAML type, write the default into {@link
 *       Oneblock#config} and return that default.
 *   <li>If the key is present and the right type, return the stored value without touching the
 *       config.
 * </ol>
 *
 * <p>Phase 6 will rename a number of YAML keys; this test pins the contract so a regression in the
 * read-or-write fallback (for instance forgetting the {@code config.set(...)} call, or missing the
...
class ConfigManagerReadOrDefaultTest {
  ...
  @Test
  @DisplayName(
      "readOrDefault(List<String>): missing key writes the default, returns its YAML readback")
