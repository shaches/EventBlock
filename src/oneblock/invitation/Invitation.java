...
public class Invitation extends AbstractInvitation {
  /**
   * Pending invites awaiting {@code /ob accept}. Phase 4.2 swapped the previous {@code ArrayList}
   * for a {@link CopyOnWriteArrayList} so that the {@link #add(UUID, UUID)} duplicate scan, the
   * {@link #check(UUID)} and {@link #check(org.bukkit.entity.Player)} lookups, and the trailing
   * {@code list.remove(inv_)} (delivered as a {@code runTaskLater} lambda 300 ticks after creation)
   * no longer share an internal iterator with any concurrent mutator. All current writers remain on
   * the main thread; this is the same forward-looking guard described in {@link Guest#list}.
   */
  ...
  public static void add(UUID name, UUID to) {
    for (Invitation item : Invitation.list) if (item.equals(name, to)) return;
  ...
  public static Invitation check(UUID uuid) {
    for (Invitation item : list) if (item.Invited.equals(uuid)) return item;
    return null;
  }
