  ...
  public boolean execute(CommandContext ctx) {
    ...
    if (isColor || args[1].equalsIgnoreCase("style")) {
      if (args.length == 2) {
        ctx.sender()
            .sendMessage(
                String.format("%senter a %s name.", ChatColor.YELLOW, args[1].toLowerCase()));
        return true;
      }
      try {
        if (isColor) {
        ...
        ctx.sender()
            .sendMessage(
                String.format(
                    "%sProgress bar %s = %s", ChatColor.GREEN, args[1].toLowerCase(), args[2]));
      } catch (Exception e) {
        ctx.sender()
            .sendMessage(
                String.format(
                    "%sPlease enter a valid %s. For example: %s",
                    ChatColor.YELLOW, args[1].toLowerCase(), isColor ? "RED" : "SOLID"));
      }
      return true;
    }
