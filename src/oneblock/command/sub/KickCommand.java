...
/**
 * {@code /ob kick <player>} - user-facing. The island owner removes a co-owner; if the kicked
 * player is currently online and standing on the island, they are also bounced back to the world
 * spawn via {@code /ob j} (which routes through their own join logic).
 *
 * <p>Behaviour-equivalent to the legacy {@code "kick"} switch case extracted in Phase 3.5b.
 * Requires {@code Oneblock.kick} and a {@link Player} sender (the owner). The owner cannot kick
 * themselves; the kick is also a no-op if the named player is not actually on the sender's invite
 * list.
 */
public final class KickCommand implements Subcommand {
  ...
  public boolean execute(CommandContext ctx) {
    ...
    if (!PlayerInfo.existsAsOwner(owner_uuid)) return true;
    ...
    if (info.uuids.contains(member_uuid)) {
      ...
      if (OBWorldGuard.isEnabled()) plugin.worldGuard.removeMember(member_uuid, ownerID);
    }
    ...
    if (memberID == ownerID) {
      if (!member_ex.hasPermission("Oneblock.set")) member_ex.performCommand("ob j");
