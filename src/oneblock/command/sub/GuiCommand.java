...
/**
 * {@code /ob gui} - dual-purpose. With no args, opens the user-facing island GUI for the sender
 * (player-only). With one or more args (e.g. {@code /ob gui true|false}), it's the admin
 * bool-toggle for the {@code gui} flag, gated behind {@code Oneblock.set}.
 *
 * <p>Behaviour-equivalent to the legacy {@code "gui"} switch case + its fall-through chain to the
 * admin {@code default} block, extracted in Phase 3.5b.4. The user-facing path requires no extra
 * permission (the umbrella {@code Oneblock.join} suffices); the admin path defends against
 * non-admins internally.
 */
