...
/**
 * {@code /ob clear <player>} - admin. Wipes a player's island level and breaks counter back to
 * zero, hides their BossBar, and erases their island geometry (replacing it with air around the
 * centre block out to {@code offset/4} radius). Does not remove the {@link PlayerInfo} entry;
 * {@code /ob idreset} is the harder reset that also frees the slot.
 *
 * <p>Behaviour-equivalent to the legacy {@code "clear"} admin case extracted in Phase 3.5b.
 */
public final class ClearCommand implements Subcommand {
  ...
  public boolean execute(CommandContext ctx) {
    ...
    if (args.length <= 1) {
      ctx.sender()
          .sendMessage(String.format("%sinvalid format. try: /ob clear 'nickname'", ChatColor.RED));
      return true;
    }
