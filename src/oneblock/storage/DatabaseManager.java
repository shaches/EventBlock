...
public class DatabaseManager {
  ...
  private static void addColumnIfMissing(String columnName, String columnDef) {
    ...
    try (Connection conn = dataSource.getConnection()) {
      ...
      try (ResultSet rs = meta.getColumns(null, null, "PLAYER_DATA", columnName.toUpperCase())) {
        if (rs.next()) return;
      }
      ...
      Oneblock.plugin.getLogger().log(Level.SEVERE, "Failed to add column " + columnName, e);
    }
  }
  ...
  public static List<PlayerInfo> load() {
    ...
    try (Connection conn = dataSource.getConnection();
      ...
      while (rs.next()) {
        ...
        UUID uuid =
            (uuidStr != null && !uuidStr.trim().isEmpty()) ? UUID.fromString(uuidStr.trim()) : null;
  ...
  public static boolean save(List<PlayerInfo> players) {
    ...
    String upsertSQL =
        "INSERT INTO player_data (island_id, uuid, level, breaks, current_level_id, task_progress,"
            + " allow_visit, invited_players) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY"
            + " UPDATE uuid = VALUES(uuid), level = VALUES(level), breaks = VALUES(breaks),"
            + " current_level_id = VALUES(current_level_id), task_progress = VALUES(task_progress),"
            + " allow_visit = VALUES(allow_visit), invited_players = VALUES(invited_players)";
