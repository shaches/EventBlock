 ...
 * LootTable} referenced by {@link NamespacedKey}. Guarded against pre-1.9 servers where the
 * LootTable API is unavailable.
 */
public class LootTableDispatcher {
  private static final Logger LOG = Bukkit.getLogger();
  private static final NamespacedKey FALLBACK_KEY =
  ...
  public static boolean populate(Block block, NamespacedKey key, Random rnd) {
    ...
    if (Compat.superlegacy) {
      if (!legacyWarned) {
        LOG.warning(
            "[Oneblock] LootTable API is unsupported on this server version; chests will spawn"
                + " empty.");
        legacyWarned = true;
      }
      return true;
    ...
    if (table == null) {
      LOG.warning(
          "[Oneblock] Loot table '"
              + key
              + "' not found; using vanilla fallback '"
              + FALLBACK_KEY
              + "'.");
